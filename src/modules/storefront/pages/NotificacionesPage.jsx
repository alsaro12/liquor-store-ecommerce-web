...
const TIPO_META = {
  pedido: { emoji: "📦", color: "#1c4593", bg: "#d8e8ff" },
  club: { emoji: "🏆", color: "#8a5a06", bg: "#fff3d2" },
  promo: { emoji: "🎉", color: "#94381f", bg: "#fbd9d2" },
  sistema: { emoji: "ℹ️", color: "#5b3c1a", bg: "#ece4d4" }
};
...
export default function NotificacionesPage({ onUnreadChange, onNavigate }) {
  ...
  async function refresh() {
    ...
    setError("");
    try {
      const list = await listNotificaciones({ tipo: tab });
  ...
  function renderItem(item) {
    ...
    return (
      <li key={item.id} className={`notif-item${item.leida ? "" : " is-unread"}`}>
    ...
    <section className="page-shell">
      <header className="page-head page-head-row">
        <div>
          ...
          <p>Mantente al tanto de tus pedidos, beneficios del club y novedades.</p>
        </div>
        {unreadCount ? (
      ...
      {loading ? (
        ...
        <div className="page-empty">
          ...
          <p>Cuando hagas un pedido o aparezcan promos verás aquí todo lo importante.</p>
        </div>
      ) : (
        <>
          {newer.length > 0 ? (
