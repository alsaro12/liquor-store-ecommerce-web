...
import { AREQUIPA_CENTER, geohashEncode, isInsideArequipa, reverseGeocode, searchPlaces } from "./direccionesApi.js";

// Arequipa centro como fallback inicial.
const DEFAULT_CENTER = AREQUIPA_CENTER;
...
function resolveInitialCenter(initial) {
  const candidate = {
    lat: Number(initial?.latitud),
    lng: Number(initial?.longitud)
  };
...
export default function AddressPicker({ open, initial, onClose, onSelect }) {
  ...
  const [center, setCenter] = useState(() => resolveInitialCenter(initial));
  ...
  useEffect(() => {
    ...
    map.on("click", (event) => {
      ...
      if (!isInsideArequipa({ lat, lng })) return;
      setMapMoved(true);
      ...
      <div className="address-picker">
        ...
        <div className="address-picker-map-wrap">
          ...
          <p className="address-picker-hint">
            {mapMoved ? "Ubicación confirmada por movimiento del mapa." : "Mueve el mapa para colocar el pin en tu puerta."}
          </p>
        </div>
        ...
        <div className="address-picker-actions">
          ...
          <button type="button" className="address-picker-save" onClick={handleSave} disabled={loadingReverse || !mapMoved}>
            Guardar ubicación
          </button>
        </div>
      </div>
