...
function createProductosObjectServer(deps) {
  ...
  return async function handleProductosObjectRoute(req, res, pathname, query) {
    ...
    const imageProductMatch = pathname.match(/^\/api\/productos\/imagen-producto\/(\d+)\/?$/);
    if (imageProductMatch) {
      if (req.method !== "GET") {
        sendText(res, 405, "Metodo no permitido.");
        return true;
      }
      const record = typeof readProductImageByProductId === "function"
        ? await readProductImageByProductId(imageProductMatch[1])
        : null;
      if (!record || !record.source) {
        sendText(res, 404, "Imagen no encontrada.");
        return true;
