...
function normalizeOrderStatus(value) {
  ...
  if (normalized === "pago_verificado") return "PAGO_VERIFICADO";
  if (normalized === "en_preparacion") return "EN_PREPARACION";
...
async function createOrder(payload, options = {}) {
  ...
  const order = buildOrderApiShape({
    id: trimValue(payload?.id || `LC-${Date.now()}`),
  ...
  if (order.usuarioId) {
    try {
      await addNotificacion(order.usuarioId, {
        ...
        mensaje: `Tu pedido ${order.id} fue confirmado. Te avisaremos cuando esté en camino.`,
...
async function updateOrder(orderId, payload) {
  ...
  const next = buildOrderApiShape({
    ...existing,
    status: payload?.status !== undefined ? normalizeOrderStatus(payload.status) : existing.status,
  ...
  return withDataSourceFallback(
    ...
    () =>
      withMysqlConnection(async (connection) => {
        ...
        const [rows] = await connection.query(
          `SELECT id, nombre, descripcion, categoria, precio, stock_actual, estado, imagenes_json
           FROM productos
        ...
        const [imgRows] = await connection.query(
          `SELECT id, producto_id, hash_image, thumb_webp_url, original_webp_url
        ...
        return safeRows.map((row) => {
          ...
          return {
            ...
            imageHash: coverByProduct.get(toInt(row?.id, 0)) || "",
    ...
    async () => {
      ...
      return rows.map(csvProductToApi).map((row) => ({
...
const CLUB_NIVELES = [
  { nivel: "BRONCE", min: 0, max: 299, color: "#a3680b" },
  { nivel: "PLATA", min: 300, max: 999, color: "#7a7a7a" },
  { nivel: "ORO", min: 1000, max: 9999999, color: "#c89200" }
];
...
function resolveClubNivel(puntos) {
  ...
  const objetivo = siguiente ? siguiente.min : current.max;
  const base = current.min;
  const span = Math.max(1, objetivo - base);
  const progreso = Math.max(0, Math.min(1, (safe - base) / span));
  return {
    nivel: current.nivel,
    color: current.color,
    puntos: safe,
    siguiente: siguiente?.nivel || null,
    faltante: siguiente ? Math.max(0, siguiente.min - safe) : 0,
    progreso
  };
}
...
async function buildCuentaResumen(user) {
  ...
  const { codigo, puntos } = await getReferidoInfo(userId);
  ...
  try {
    ...
    if (productIds.length) {
      const products = await withMysqlConnection(async (connection) => {
        await ensureProductImagesTable(connection);
        const placeholders = productIds.map(() => "?").join(",");
        const [rows] = await connection.query(
          `SELECT id, nombre, categoria, precio FROM productos WHERE id IN (${placeholders})`,
          productIds
        );
        const [imgRows] = await connection.query(
          `SELECT producto_id, hash_image FROM producto_imagenes
           WHERE producto_id IN (${placeholders}) AND es_portada = 1`,
          productIds
        );
        const imgMap = new Map();
        for (const ir of Array.isArray(imgRows) ? imgRows : []) {
          imgMap.set(Number(ir.producto_id), ir.hash_image || "");
        }
        return (Array.isArray(rows) ? rows : []).map((row) => ({
          id: String(row.id),
          name: trimValue(row.nombre || ""),
          category: trimValue(row.categoria || ""),
          price: round2(row.precio),
          imageHash: imgMap.get(Number(row.id)) || ""
        }));
      });
  ...
  const club = resolveClubNivel(puntos);
  const beneficios = [
    {
      nivel: "BRONCE",
      color: "#a3680b",
      bullets: ["Promos exclusivas", "Recompensa al cumpleaños", "Notificaciones prioritarias"]
    },
    {
      nivel: "PLATA",
      color: "#7a7a7a",
      bullets: ["Todo lo de BRONCE", "Delivery gratis 2 veces al mes", "Cupones sorpresa"]
    },
    {
      nivel: "ORO",
      color: "#c89200",
      bullets: ["Todo lo de PLATA", "Delivery prioritario", "Acceso early a combos limitados"]
    }
  ];
...
async function getReferidoInfo(usuarioId) {
  return withMysqlConnection(async (connection) => {
    ...
    const [rows] = await connection.query(
      "SELECT nombre, codigo_referido, puntos_club FROM usuarios_cliente WHERE id = ? LIMIT 1",
      [usuarioId]
    );
    const row = Array.isArray(rows) && rows.length ? rows[0] : null;
    if (!row) return { codigo: "", puntos: 0 };

    let codigo = row.codigo_referido || "";
    // Autogenerar si falta (usuarios pre-migración)
    if (!codigo) {
      for (let attempt = 0; attempt < 6; attempt += 1) {
        const candidate = generateReferralCode(row.nombre);
      ...
      if (codigo) {
        try {
          await connection.query(
            "UPDATE usuarios_cliente SET codigo_referido = ? WHERE id = ?",
            [codigo, usuarioId]
          );
        } catch (_) {}
      }
    ...
    return { codigo, puntos: Number(row.puntos_club || 0) };
  });
}
...
async function listInvitacionesByUser(usuarioId) {
  return withMysqlConnection(async (connection) => {
    ...
    const [rows] = await connection.query(
      `SELECT * FROM cliente_invitaciones WHERE usuario_id = ? ORDER BY created_at DESC LIMIT 200`,
      [usuarioId]
    );
    return (Array.isArray(rows) ? rows : []).map(buildInvitacionShape);
  });
}
  ...
  return withMysqlConnection(async (connection) => {
    ...
    const [result] = await connection.query(
      `INSERT INTO cliente_invitaciones
        (usuario_id, email, destinatario_nombre, estado, premio_puntos)
       VALUES (?, ?, ?, 'enviada', 0)`,
      [usuarioId, email || null, nombre || null]
    );
    const [rows] = await connection.query(
      "SELECT * FROM cliente_invitaciones WHERE id = ? LIMIT 1",
      [result.insertId]
    );
    return buildInvitacionShape(rows[0]);
  });
}
...
async function promoteInvitacionPrimerPedido(usuarioReferidoId) {
  return withMysqlConnection(async (connection) => {
    await ensureClienteInvitacionesTable(connection);
    const [rows] = await connection.query(
      `SELECT id, usuario_id, estado FROM cliente_invitaciones
       WHERE referido_usuario_id = ? ORDER BY id DESC LIMIT 1`,
      [usuarioReferidoId]
    );
    const inv = Array.isArray(rows) && rows.length ? rows[0] : null;
    if (!inv || inv.estado === "recompensa_otorgada") return null;
    await connection.query(
      `UPDATE cliente_invitaciones
       SET estado = 'recompensa_otorgada', premio_puntos = ?
       WHERE id = ?`,
      [REFERIDO_PREMIO_PUNTOS, inv.id]
    );
    await connection.query(
      "UPDATE usuarios_cliente SET puntos_club = puntos_club + ? WHERE id = ?",
      [REFERIDO_PREMIO_PUNTOS, inv.usuario_id]
    );
    try {
      await addNotificacion(inv.usuario_id, {
        tipo: "club",
    ...
    return { invitacionId: inv.id, referrerId: inv.usuario_id, premio: REFERIDO_PREMIO_PUNTOS };
  });
}
...
async function registerCustomer(payload) {
  ...
  const telefono = trimValue(payload?.telefono || "").slice(0, 40);
  ...
  return withMysqlConnection(async (connection) => {
    ...
    const [existing] = await connection.query(
      "SELECT id FROM usuarios_cliente WHERE email = ? LIMIT 1",
      [email]
    );
    ...
    const [result] = await connection.query(
      `INSERT INTO usuarios_cliente
        (email, password_hash, nombre, telefono, codigo_referido, referido_por)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [email, passwordHash, nombre, telefono, codigoRef, refCode || null]
    );
    ...
    return {
      token: session.token,
      user: { id: userId, email, nombre, telefono, codigo_referido: codigoRef }
    };
  });
}
...
async function loginCustomer(payload) {
  ...
  const password = String(payload?.password || "");
  if (!email || !password) {
    throw createHttpError(400, "Faltan credenciales.");
  }
  ...
  return withMysqlConnection(async (connection) => {
    ...
    const [rows] = await connection.query(
      "SELECT id, email, password_hash, nombre, telefono, codigo_referido, puntos_club FROM usuarios_cliente WHERE email = ? LIMIT 1",
      [email]
    );
    ...
    return {
      ...
      user: {
        ...
        puntos_club: Number(user.puntos_club || 0)
      }
    };
  });
}
...
async function findCustomerByToken(token) {
  ...
  return withMysqlConnection(async (connection) => {
    ...
    await ensureSesionesTable(connection);
    const [rows] = await connection.query(
      `SELECT u.id, u.email, u.nombre, u.telefono, u.codigo_referido, u.puntos_club, s.expires_at
...
function validateDireccionInput(input) {
  const direccion = trimValue(input?.direccion || "").slice(0, 255);
...
async function listDireccionesByUser(usuarioId) {
  return withMysqlConnection(async (connection) => {
    await ensureClienteDireccionesTable(connection);
    const [rows] = await connection.query(
    ...
    return (Array.isArray(rows) ? rows : []).map(buildDireccionApiShape);
  });
}
...
async function getDireccionById(usuarioId, id) {
  return withMysqlConnection(async (connection) => {
    await ensureClienteDireccionesTable(connection);
    const [rows] = await connection.query(
      "SELECT * FROM cliente_direcciones WHERE id = ? AND usuario_id = ? LIMIT 1",
      [id, usuarioId]
    );
    const row = Array.isArray(rows) && rows.length ? rows[0] : null;
    return row ? buildDireccionApiShape(row) : null;
  });
}

async function createDireccion(usuarioId, input) {
  const data = validateDireccionInput(input);
  return withMysqlConnection(async (connection) => {
    await ensureClienteDireccionesTable(connection);
    const [existing] = await connection.query(
      "SELECT COUNT(*) AS total FROM cliente_direcciones WHERE usuario_id = ?",
      [usuarioId]
    );
    const totalExisting = Number(existing?.[0]?.total || 0);
    const setPrincipal = data.es_principal || totalExisting === 0;

    if (setPrincipal) {
      await connection.query(
        "UPDATE cliente_direcciones SET es_principal = 0 WHERE usuario_id = ?",
        [usuarioId]
      );
    }
    const [result] = await connection.query(
      `INSERT INTO cliente_direcciones
        (usuario_id, etiqueta, icono, direccion, referencia, distrito, ciudad, telefono,
         latitud, longitud, geohash, es_principal)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        usuarioId,
        data.etiqueta,
    ...
    return getDireccionById(usuarioId, result.insertId);
  });
}
  ...
  return withMysqlConnection(async (connection) => {
    ...
    const [result] = await connection.query(
      `UPDATE cliente_direcciones
       SET etiqueta = ?, icono = ?, direccion = ?, referencia = ?,
           distrito = ?, ciudad = ?, telefono = ?,
    ...
    if (!result.affectedRows) throw createHttpError(404, "Dirección no encontrada.");
    return getDireccionById(usuarioId, id);
  });
}
...
async function deleteDireccion(usuarioId, id) {
  await withMysqlConnection(async (connection) => {
    await ensureClienteDireccionesTable(connection);
    const [result] = await connection.query(
      "DELETE FROM cliente_direcciones WHERE id = ? AND usuario_id = ?",
      [id, usuarioId]
    );
    if (!result.affectedRows) throw createHttpError(404, "Dirección no encontrada.");
    // si dejó al usuario sin principal, marcar la más reciente como principal
    const [remaining] = await connection.query(
      `SELECT id FROM cliente_direcciones
       WHERE usuario_id = ?
    ...
    if (Array.isArray(remaining) && remaining.length) {
      ...
      const [countPrincipal] = await connection.query(
        "SELECT COUNT(*) AS total FROM cliente_direcciones WHERE usuario_id = ? AND es_principal = 1",
        [usuarioId]
      );
      if (Number(countPrincipal?.[0]?.total || 0) === 0) {
        await connection.query(
          "UPDATE cliente_direcciones SET es_principal = 1 WHERE id = ?",
          [next]
        );
      }
    }
...
async function setDireccionPrincipal(usuarioId, id) {
  return withMysqlConnection(async (connection) => {
    await ensureClienteDireccionesTable(connection);
    const [rows] = await connection.query(
      "SELECT id FROM cliente_direcciones WHERE id = ? AND usuario_id = ? LIMIT 1",
      [id, usuarioId]
    );
    if (!Array.isArray(rows) || !rows.length) {
      throw createHttpError(404, "Dirección no encontrada.");
    }
    await connection.query(
      "UPDATE cliente_direcciones SET es_principal = 0 WHERE usuario_id = ?",
      [usuarioId]
...
async function listFavoritosByUser(usuarioId) {
  return withMysqlConnection(async (connection) => {
    await ensureClienteFavoritosTable(connection);
    const [rows] = await connection.query(
    ...
    return (Array.isArray(rows) ? rows : []).map((row) => ({
      id: toInt(row.id, 0),
      tipo: String(row.tipo || "producto"),
      referencia_id: String(row.referencia_id || ""),
      created_at: row.created_at ? new Date(row.created_at).toISOString() : null
    }));
  });
}
...
async function addFavorito(usuarioId, payload) {
  const data = normalizeFavoritoInput(payload);
  return withMysqlConnection(async (connection) => {
    await ensureClienteFavoritosTable(connection);
    try {
      const [result] = await connection.query(
        "INSERT INTO cliente_favoritos (usuario_id, tipo, referencia_id) VALUES (?, ?, ?)",
        [usuarioId, data.tipo, data.referencia_id]
      );
      return { id: result.insertId, tipo: data.tipo, referencia_id: data.referencia_id };
    } catch (error) {
      if (error?.code === "ER_DUP_ENTRY") {
        const [rows] = await connection.query(
          "SELECT id, tipo, referencia_id FROM cliente_favoritos WHERE usuario_id = ? AND tipo = ? AND referencia_id = ? LIMIT 1",
          [usuarioId, data.tipo, data.referencia_id]
        );
        const row = Array.isArray(rows) && rows.length ? rows[0] : null;
        if (row) return { id: row.id, tipo: row.tipo, referencia_id: row.referencia_id };
      }
      throw error;
    }
  });
...
async function removeFavoritoById(usuarioId, id) {
  await withMysqlConnection(async (connection) => {
    await ensureClienteFavoritosTable(connection);
    const [result] = await connection.query(
      "DELETE FROM cliente_favoritos WHERE id = ? AND usuario_id = ?",
      [id, usuarioId]
    );
    if (!result.affectedRows) throw createHttpError(404, "Favorito no encontrado.");
  });
}
...
async function removeFavoritoByRef(usuarioId, tipo, referenciaId) {
  await withMysqlConnection(async (connection) => {
    await ensureClienteFavoritosTable(connection);
    await connection.query(
      "DELETE FROM cliente_favoritos WHERE usuario_id = ? AND tipo = ? AND referencia_id = ?",
...
const COMBO_TIPOS = new Set([
  "pre", "playa", "chill", "fiesta", "romantico", "premium",
  "mixers", "after", "general"
]);
...
const COMBO_ITEM_RECIPES = {
  "combo-pre": [
    { categorias: ["CERVEZA"], cantidad: 12 },
    { categorias: ["HIELO"], cantidad: 1 },
    { categorias: ["SNACK"], cantidad: 2 }
  ],
  "combo-playa": [
    { categorias: ["CERVEZA"], cantidad: 18 },
  ...
  "combo-full": [
    ...
    { categorias: ["ENERGIZANTE"], cantidad: 4 },
    { categorias: ["HIELO"], cantidad: 1 }
  ],
  "combo-romantico": [
    { categorias: ["VINO", "ESPUMANTE"], cantidad: 1 },
    { categorias: ["SNACK"], cantidad: 2 }
  ],
  "combo-premium": [
    { categorias: ["WHISKY"], cantidad: 1 },
  ...
  "combo-after": [
    { categorias: ["RON"], cantidad: 1 },
    { categorias: ["CERVEZA"], cantidad: 6 },
    { categorias: ["SNACK"], cantidad: 2 },
    { categorias: ["HIELO"], cantidad: 1 }
  ],
  "combo-gin-tonic": [
    { categorias: ["GIN"], cantidad: 1 },
    { categorias: ["GASEOSA", "AGUA"], cantidad: 4 },
    { categorias: ["SNACK"], cantidad: 2 }
  ]
};
...
async function ensureDefaultComboItemsSeed(connection) {
  ...
  for (const combo of comboRows) {
    ...
    for (const ingr of recipe) {
      // Intenta primero con las categorías ideales, luego con OTRO como último recurso
      let productId = await pickProductForCategorias(ingr.categorias, productosUsados);
      if (!productId) {
        productId = await pickProductForCategorias(["OTRO"], productosUsados);
      }
    ...
    () =>
      withMysqlConnection(async (connection) => {
        ...
        const [itemRows] = await connection.query(
          `SELECT combo_id, producto_id, cantidad, orden
           FROM combo_items
...
async function getComboBySlug(slug) {
  ...
  return withMysqlConnection(async (connection) => {
    ...
    if (!row) return null;
    const [itemRows] = await connection.query(
      "SELECT producto_id, cantidad, orden FROM combo_items WHERE combo_id = ? ORDER BY orden ASC, id ASC",
      [row.id]
    );
...
async function listMetodosPagoByUser(usuarioId) {
  return withMysqlConnection(async (connection) => {
    await ensureClienteMetodosPagoTable(connection);
    const [rows] = await connection.query(
      `SELECT * FROM cliente_metodos_pago
       WHERE usuario_id = ?
       ORDER BY es_principal DESC, created_at DESC`,
      [usuarioId]
    );
    return (Array.isArray(rows) ? rows : []).map(buildMetodoPagoShape);
  });
}
...
async function getMetodoPagoById(usuarioId, id) {
  return withMysqlConnection(async (connection) => {
    await ensureClienteMetodosPagoTable(connection);
    const [rows] = await connection.query(
      "SELECT * FROM cliente_metodos_pago WHERE id = ? AND usuario_id = ? LIMIT 1",
      [id, usuarioId]
    );
    const row = Array.isArray(rows) && rows.length ? rows[0] : null;
    return row ? buildMetodoPagoShape(row) : null;
  });
}

async function createMetodoPago(usuarioId, input) {
  const data = validateMetodoPagoInput(input);
  return withMysqlConnection(async (connection) => {
    await ensureClienteMetodosPagoTable(connection);

    // Si ya existe ese número para este usuario en el mismo tipo, evitar duplicado
    const [existing] = await connection.query(
      "SELECT id FROM cliente_metodos_pago WHERE usuario_id = ? AND tipo = ? AND numero = ? LIMIT 1",
      [usuarioId, data.tipo, data.numero]
    );
    if (Array.isArray(existing) && existing.length) {
    ...
    const [countRows] = await connection.query(
      "SELECT COUNT(*) AS total FROM cliente_metodos_pago WHERE usuario_id = ?",
      [usuarioId]
    );
    const setPrincipal = data.es_principal || Number(countRows?.[0]?.total || 0) === 0;
    if (setPrincipal) {
      await connection.query(
        "UPDATE cliente_metodos_pago SET es_principal = 0 WHERE usuario_id = ?",
        [usuarioId]
      );
    }
    const [result] = await connection.query(
      `INSERT INTO cliente_metodos_pago
        (usuario_id, tipo, proveedor, alias, numero, ultimos4, identificador_encriptado, verificado, es_principal)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        usuarioId,
        data.tipo,
        ...
        data.numero,
        data.ultimos4,
        data.identificador_encriptado,
        data.verificado,
        setPrincipal ? 1 : 0
      ]
    );
    return getMetodoPagoById(usuarioId, result.insertId);
  });
}
...
async function deleteMetodoPago(usuarioId, id) {
  return withMysqlConnection(async (connection) => {
    await ensureClienteMetodosPagoTable(connection);
    const [result] = await connection.query(
      "DELETE FROM cliente_metodos_pago WHERE id = ? AND usuario_id = ?",
      [id, usuarioId]
    );
    if (!result.affectedRows) throw createHttpError(404, "Método no encontrado.");

    // si quedó sin principal, marcar el más reciente
    const [remaining] = await connection.query(
      "SELECT id FROM cliente_metodos_pago WHERE usuario_id = ? ORDER BY created_at DESC LIMIT 1",
      [usuarioId]
    );
    if (Array.isArray(remaining) && remaining.length) {
      const [hasPrincipal] = await connection.query(
        "SELECT COUNT(*) AS total FROM cliente_metodos_pago WHERE usuario_id = ? AND es_principal = 1",
        [usuarioId]
      );
      if (Number(hasPrincipal?.[0]?.total || 0) === 0) {
        await connection.query(
          "UPDATE cliente_metodos_pago SET es_principal = 1 WHERE id = ?",
          [remaining[0].id]
        );
      }
    }
...
async function setMetodoPagoPrincipal(usuarioId, id) {
  return withMysqlConnection(async (connection) => {
    await ensureClienteMetodosPagoTable(connection);
    const [rows] = await connection.query(
      "SELECT id FROM cliente_metodos_pago WHERE id = ? AND usuario_id = ? LIMIT 1",
      [id, usuarioId]
    );
    if (!Array.isArray(rows) || !rows.length) {
      throw createHttpError(404, "Método no encontrado.");
    }
    await connection.query(
      "UPDATE cliente_metodos_pago SET es_principal = 0 WHERE usuario_id = ?",
      [usuarioId]
    ...
    return getMetodoPagoById(usuarioId, id);
  });
}
...
async function addNotificacion(usuarioId, payload) {
  ...
  return withMysqlConnection(async (connection) => {
    await ensureClienteNotificacionesTable(connection);
    const [result] = await connection.query(
      `INSERT INTO cliente_notificaciones (usuario_id, tipo, titulo, mensaje, icono, link)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [usuarioId, tipo, titulo, mensaje, icono, link]
    );
    return { id: result.insertId, tipo, titulo, mensaje, icono, link, leida: false };
  });
}
...
async function listNotificacionesByUser(usuarioId, { soloNoLeidas = false, tipo = "" } = {}) {
  return withMysqlConnection(async (connection) => {
    ...
    if (tipo && NOTIF_TIPOS.has(String(tipo).toLowerCase())) {
      where += " AND tipo = ?";
      params.push(String(tipo).toLowerCase());
    }
    const [rows] = await connection.query(
      `SELECT * FROM cliente_notificaciones ${where} ORDER BY leida ASC, created_at DESC LIMIT 200`,
      params
    );
    return (Array.isArray(rows) ? rows : []).map(buildNotificacionShape);
  });
}
...
async function countUnreadNotificaciones(usuarioId) {
  return withMysqlConnection(async (connection) => {
    await ensureClienteNotificacionesTable(connection);
    const [rows] = await connection.query(
      "SELECT COUNT(*) AS total FROM cliente_notificaciones WHERE usuario_id = ? AND leida = 0",
      [usuarioId]
    );
...
async function markNotificacionLeida(usuarioId, id) {
  await withMysqlConnection(async (connection) => {
    await ensureClienteNotificacionesTable(connection);
    const [result] = await connection.query(
      "UPDATE cliente_notificaciones SET leida = 1 WHERE id = ? AND usuario_id = ?",
      [id, usuarioId]
    );
    if (!result.affectedRows) throw createHttpError(404, "Notificación no encontrada.");
  });
}
...
async function markAllNotificacionesLeidas(usuarioId) {
  return withMysqlConnection(async (connection) => {
    await ensureClienteNotificacionesTable(connection);
    const [result] = await connection.query(
      "UPDATE cliente_notificaciones SET leida = 1 WHERE usuario_id = ? AND leida = 0",
      [usuarioId]
    );
...
async function getProductCategories() {
  return Array.from(PRODUCT_CATEGORIES.values());
}
...
async function handleStorefrontMyOrderById(req, res, codigo) {
  ...
  const order = all.find(
    (entry) => entry.id === target && Number(entry.usuarioId) === Number(user.id)
  );
  if (!order) {
