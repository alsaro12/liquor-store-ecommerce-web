...
function normalizeSaleOrigin(value) {
  ...
  if (["app", "aplicacion", "aplicaciÃ³n"].includes(normalized)) return "APP";
...
async function readProductMovementsHistory(productIdInput) {
  ...
  return items
    .filter((item) => toInt(item["N°"] ?? item["N°"], 0) === productId)
...
async function createProduct(payload) {
  return withDataSourceFallback(
    ...
    await ensureProductPurchasePriceHistoryTable(connection);
    const hasCustomId =
      payload["N°"] !== undefined || payload["N°"] !== undefined || payload.id !== undefined || payload.n !== undefined;
    ...
    let id;
    if (hasCustomId) {
      id = parsePositiveInt(payload["N°"] ?? payload["N°"] ?? payload.id ?? payload.n, "N°");
    } else {
  ...
  return withDataSourceFallback(
    ...
    try {
      ...
      const nextId = hasNextId
        ? parsePositiveInt(payload["N°"] ?? payload["NÂ°"] ?? payload.id ?? payload.n, "N°")
        : id;
  ...
  return withDataSourceFallback(
    ...
    try {
      ...
      if (normalizeText(currentSale.estado || "ACTIVA") === "anulada") {
        throw createHttpError(409, `No puedes editar la venta #${saleId} porque estÃ¡ ANULADA.`);
      }
...
async function deleteSale(idInput, payload = {}) {
  ...
  return withDataSourceFallback(
    ...
    try {
      ...
      if (!currentSale) throw createHttpError(404, `No existe venta #${saleId}.`);
      if (normalizeText(currentSale.estado || "ACTIVA") === "anulada") {
        throw createHttpError(409, `La venta #${saleId} ya estÃ¡ ANULADA.`);
      }
      const cancelReason = truncateText(
        payload.motivo || payload.anulada_motivo || payload.reason || `AnulaciÃ³n manual venta #${saleId}`,
        255
      );
...
async function getDbStatus() {
  ...
  try {
    ...
    if (shouldUseLocalCsvFallback(error)) {
      ...
      try {
        ...
        return {
          ...
          missingKeys: [],
          probeMs: Date.now() - started,
          message: "No se pudo validar conexiÃ³n de base de datos ni activar respaldo local.",
    ...
    return {
      ...
      missingKeys: [],
      probeMs: Date.now() - started,
      message: "No se pudo validar conexiÃ³n de base de datos.",
      error: buildDbErrorMessage(error)
    };
  } finally {
    try {
...
async function buildDailySalesExportCsv(connection, options = {}) {
  ...
  if (from > to) {
    throw createHttpError(400, "Rango invÃ¡lido: la fecha Desde no puede ser mayor que Hasta.");
  }
