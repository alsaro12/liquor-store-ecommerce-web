window.AppBootstrapLoaded = true;
...
async function copyTextToClipboard(text) {
  ...
  input.remove();
  if (!copied) {
    throw new Error("Tu navegador bloqueó la copia automática.");
  }
}
...
async function apiRequest(path, options = {}) {
  ...
  try {
    ...
    if (error?.name === "AbortError") {
      throw new Error(`La solicitud tardó demasiado (${Math.round(timeoutMs / 1000)} s). Intenta de nuevo.`);
    }
    throw new Error(
...
function getOnboardingSteps() {
  return [
    ...
    },
    {
      ...
      body: "Escribe código o nombre y selecciona un producto. Se agrega automáticamente al carrito.",
    ...
    },
    {
      ...
      body: "En Productos en esta venta puedes subir, bajar o quitar cada ítem del carrito.",
    ...
    },
    {
      ...
      body: "Revisa el total y confirma la venta. El sistema validará que los pagos sumen exacto.",
...
function renderOnboardingStep() {
  ...
  refs.onboardingBody.textContent = target
    ? step.body
    : `${step.body} (Si no aparece, revisa conexión/productos y vuelve a intentar).`;
  refs.onboardingPrevBtn.disabled = state.onboardingStepIndex === 0;
  refs.onboardingNextBtn.textContent = state.onboardingStepIndex === maxIndex ? "Finalizar guía" : "Siguiente";
...
function buildCashCloseStats() {
  ...
  salesInRange.forEach((item) => {
    const productCode = Number(item?.["N°"] || item?.N || item?.productId || 0);
    const productName = String(item?.NOMBRE || "").trim() || "Sin nombre";
    const catalogMatch =
      productsSource.find((product) => Number(product?.["N°"] || product?.productId || product?.id || 0) === productCode) ||
...
function buildStockOrderDraft(stats) {
  ...
  return products.map((product, index) => {
    ...
    return {
      key: `${Number(product?.["N°"] || product?.productId || 0)}-${index}`,
      productId: Number(product?.["N°"] || product?.productId || 0),
...
const RECEIPT_REFERENCE_TEMPLATES = [
  {
    ...
    lines: [
      ...
      "5 03 Hit Piña Bot Chica 32.60",
      "6 06 Six Mikes (Azul, Rojo) 150.00",
      "7 03 Six Mikes (Limón) 75.00",
      "8 04 Six Mikes Maracuyá/Mone 100.00",
      "9 02 Dos Pils lata 80.00",
      "10 02 Six Cusqueña Maltina lata 100.00",
...
async function cleanInventoryReceiptTextWithDeepSeek(file, rawText) {
  ...
  const products = getInventoryReceiptProductsSource().map((product) => ({
    productId: Number(product?.["N°"] || product?.productId || product?.id || 0),
    name: String(product?.NOMBRE || product?.nombre || ""),
    purchasePrice: Number(product?.PRECIO_COMPRA || product?.precio_compra || 0)
  }));
  const payload = await apiRequest("/api/ai/receipt/analyze", {
    method: "POST",
    timeoutMs: 120000,
    body: JSON.stringify({
      items: [
...
async function recognizeInventoryReceiptFile(file) {
  ...
  return {
    ...
    lines,
    structuredRows: [],
    visionSummary: lines.length ? "OCR local sin filas estructuradas suficientes" : "OCR local sin lectura útil",
    unsupported: false
  };
}
...
function isReceiptMeaningfulProductText(value) {
  ...
  const longAlphaTokens = alphaTokens.filter((token) => token.replace(/[^a-záéíóúñ]/gi, "").length >= 3);
  const vowelTokens = alphaTokens.filter((token) => /[aeiouáéíóú]/i.test(token));
  const weirdChars = (String(value || "").match(/[^a-z0-9\s().,+\-/%áéíóúñ]/gi) || []).length;
...
function formatReceiptFallbackLabel(row) {
  ...
  return segments.join(" · ") || "Fila detectada";
}
...
function findInventoryProductIdForReceiptRow(row, products) {
  ...
  if (explicitId && products.some((item) => Number(item?.["N°"] || item?.productId || item?.id || 0) === explicitId)) {
    return explicitId;
  }
  ...
  return Number(matched?.["N°"] || matched?.productId || matched?.id || 0) || "";
}
...
function normalizeReceiptAiRow(row, products) {
  ...
  const matchedProduct = products.find((item) => Number(item?.["N°"] || item?.productId || item?.id || 0) === Number(productId || 0)) || null;
  const quantity = Math.max(
    1,
...
function buildManualReceiptRowsFromLines(lines, fileName, products) {
  ...
  return splitReceiptReadingIntoRows(lines)
    .map((line) => {
      ...
      const label = matched
        ? `${formatReceiptFallbackLabel(parsed)} · ${String(matched?.NOMBRE || "").trim()}`
        : formatReceiptFallbackLabel(parsed);
      return {
        productId: Number(matched?.["N°"] || matched?.productId || matched?.id || 0) || "",
  ...
  return (Array.isArray(structuredRows) ? structuredRows : [])
    .map((row) => {
      ...
      const matched = canSuggestProduct
        ...
        : null;
      const label = matched
        ? `${formatReceiptFallbackLabel(row)} · ${String(matched?.NOMBRE || "").trim()}`
  ...
  return (Array.isArray(visionRows) ? visionRows : [])
    .map((row) => {
      ...
      return {
        ...
        purchasePrice,
        approved: true,
        sourceLabel: matched ? `${labelBase} · ${String(matched?.NOMBRE || "").trim()}` : labelBase,
...
function renderInventoryReceiptFilesPreview() {
  ...
  refs.inventoryReceiptFilesPreview.innerHTML = files.length
    ? files
        ...
        .join("")
    : '<div class="inventory-receipt-empty">Aún no subiste recibos.</div>';
}
...
function renderInventoryReceiptInsights() {
    ...
    .map((item) => {
      ...
      return `
        <article class="inventory-receipt-insight-item">
          ...
          <p>${esc(preview || "La IA aún no devuelve texto legible para este archivo.")}</p>
        </article>
      `;
    })
    .filter(Boolean)
  ...
  refs.inventoryReceiptInsights.innerHTML = insightItems
    ...
    : `<strong>Lectura OCR</strong><p>Sube un recibo para ver aquí la reconstrucción local de filas detectadas en la tabla del documento.</p>`;
}
...
function renderInventoryReceiptDraft() {
  ...
  const options = products
    .map((product) => {
      const productId = Number(product?.["N°"] || product?.productId || product?.id || 0);
      const selected = "";
  ...
  if (refs.inventoryReceiptSummary) {
    ...
    refs.inventoryReceiptSummary.textContent = `${detected} fila(s) detectadas · ${approved.length} producto(s) listos para confirmar.`;
  }
  if (refs.inventoryReceiptPurchaseTotal) {
    const total = rows
      .filter((row) => row.approved)
      .reduce((sum, row) => sum + Number(row.purchasePrice || 0), 0);
    refs.inventoryReceiptPurchaseTotal.textContent = money(total);
  }
}
...
async function appendInventoryReceiptRowsFromFiles(files) {
  ...
  for (const file of files) {
    ...
    file.analysisPreview = "El OCR local está reconstruyendo las filas de la tabla...";
    ...
    try {
      ...
      file.analysisPreview = previewParts.join(" · ");
      if (analysis.unsupported) {
        ...
        nextRows.push({
          ...
          purchasePrice: 0,
          approved: true,
          sourceLabel: `Revisión manual · ${file.name}`,
          rawText: file.name
        });
        continue;
      }
      ...
      if (parsedRows.length < 6) {
        ...
        if (referenceRows.length > parsedRows.length) {
          ...
          file.analysisPreview = `Plantilla de referencia aplicada · ${referenceRows.slice(0, 2).map((row) => row.rawText).join(" / ")}`;
        }
      }
      ...
      if (parsedRows.length) {
        ...
        file.analysisPreview = previewParts.join(" · ") || "No se detectaron filas estructuradas para convertir en tabla.";
      }
    } catch {
      file.analysisStatus = "Error de lectura";
      file.analysisPreview = "No se pudo extraer texto útil de esta foto con la IA en este intento.";
      nextRows.push({
        productId: "",
        quantity: inferReceiptQuantity(file.name),
        stockUnits: inferReceiptQuantity(file.name),
...
function openInventoryReceiptDialog() {
  ...
  try {
    ...
    if (refs.inventoryReceiptTurn) {
      refs.inventoryReceiptTurn.textContent = `${getCurrentTurnFilterLabel()} · ${todayInputValue()} · recibos de compra`;
    }
    renderInventoryReceiptFilesPreview();
    renderInventoryReceiptInsights();
...
async function applyInventoryReceiptIngress() {
  ...
  if (!approvedRows.length) {
    setAppMessage("Confirma al menos un producto válido para aplicar el ingreso.", "is-error");
    return;
  }
  ...
  try {
    for (const row of approvedRows) {
      ...
      const readingNote = [row.sourceLabel, row.rawText].filter(Boolean).join(" · ");
      await apiRequest(`/api/productos/${row.productId}/ingreso`, {
        ...
        body: JSON.stringify({
          ...
          nota: readingNote
            ? `Ingreso desde recibo · total compra ${money(linePurchaseTotal)} · unitario ${money(unitPurchasePrice)} · ${readingNote}`
            : `Ingreso desde recibo · total compra ${money(linePurchaseTotal)} · unitario ${money(unitPurchasePrice)}`,
